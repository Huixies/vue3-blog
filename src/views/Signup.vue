<template>
    <form @submit.prevent="handleSubmit">
        ...
        <button class="button is-primary is-pulled-right" :disabled='!usernameStatus.valid||!passwordStatus.valid'>注册</button>
    </form>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    // 密码
      ...
      return validate(password.value,[required(),length({min:5,max:20})])})
    //确认密码
      const password2 = ref('password2');
      const password2Status = computed<Status>(()=>{
      return validate(password2.value,[required(),length({min:5,max:20})])})
    ...
    return {rule,username,usernameStatus,email,emailStatus,password,passwordStatus,password2,password2Status,handleSubmit}
  }
})
</script>
